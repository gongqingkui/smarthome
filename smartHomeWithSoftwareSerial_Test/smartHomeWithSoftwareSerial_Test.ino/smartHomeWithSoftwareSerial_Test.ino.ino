/*
  ...
  1        gnd          VSS一般接地
  2        +5v          VDD接电源（+5V）
  4        8           RS RS为寄存器选择，高电平1时选择数据寄存器、低电平0时选择指令寄存器。对应Arduino12
  6        7           EE(或EN)端为使能(enable)端，写操作时，下降沿使能。读操作时，E高电平有效
  ...
  TF** MOSI - pin 11 ** MISO - pin 12 ** CLK - pin 13 ** CS - pin 14
...
void loop()
{
  ...
  nowTimeStr = nowtime();
  delay(200);
  tflog(nowTimeStr + ",HCHO," + jqStr + ",Temp," + Temp + ",Humi," + Humi);
  delay(200);
  lcd.setCursor(12, 0);
  lcd.print(jqStr);
  lcd.setCursor(2, 1);
